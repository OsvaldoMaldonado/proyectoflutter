...
class EmpleadosList extends StatelessWidget {
  ...
  EmpleadosList({Key? key,required this.empleados,required this.servicio,required this.id_servicio}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    double screenSize = MediaQuery.of(context).size.width;
    double screenHeight = MediaQuery.of(context).size.width;
    ...
    for(int i = 0; i < empleados.length; i++){
      tmp.add(
        Marker(
          ...
          position: LatLng(double.parse(empleados[i].latitud.toString()), double.parse(empleados[i].longitud.toString())),
          infoWindow: InfoWindow(title: empleados[i].nombre + " " + empleados[i].apellido),
        ),
      );
    }
    return tmp;
